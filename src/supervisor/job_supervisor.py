...
class JobSupervisor:
    ...
    def get_base_command_line(self, run: dict, job_type: JobType) -> (list, bool):
        ...
        update public."supervisor_config" set command_line='[""]', command_matrix='[""]' where id=;
        ...
        # is this an adcirc time to cog array?
        ...
        if run['status'] == JobStatus.RUNNING and run['status'] != JobStatus.ERROR:
            ...
            if job_found:
                ...
                if self.util_objs['create'].is_server_process(run['job-type']):
                    # or self.util_objs['create'].is_irods_consumer_process(run['job-type']):
                    job_status = 'Complete'
    ...
    def k8s_create_run_config(self, run: dict, job_type: JobType, command_line_params: list, extend_output_path: bool = False):
        ...
        config['JOB_NAME'] += run_id.lower().replace('_', '-')
        config['DATA_VOLUME_NAME'] += run_id.lower().replace('_', '-')
            ...
            if runs is not None:
                ...
                for run in runs:
                    ...
                    if not self.check_for_duplicate_run(run_id):
                        # make sure all the params are available.
                        # instance name and debug mode are handled here because they both affect messaging and logging.
                        ...
                        self.run_list.append(
                            {'id': run_id, 'workflow_type': workflow_type, 'stormnumber': run['run_data']['stormnumber'], 'debug': debug_mode,
                             ...
                             'run-start': dt.datetime.now(), 'physical_location': physical_location, 'workflow_jobs': workflow_jobs})
